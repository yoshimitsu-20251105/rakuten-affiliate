...
const evergreenTier = {
  ...
  subKeywords: [
    ...
    // 2026-08-28: 「国産・無添加・プレミアム志向」のフェーズ1として追加(research-log.md参照)。
    // 実在庫を検証済み(ドッグフード6,315件/キャットフード1,376件、いずれもフィルタ通過多数)。
    // マイベストとの差別化は「スコア算出式の完全公開」と「レビューを食いつきの代理指標として使う」点。
    // フェーズ2(シニア犬/小型犬/グレインフリー/おやつ)は、このフェーズ1の実績を見てから追加する。
    "国産 無添加 ドッグフード", "国産 無添加 キャットフード",
    // 2026-08-28: フェーズ1を6クラスターに拡張(実在庫検証済み)。「30日 or 累計1,000
    // インプレッション」を第一次判定基準とし、この期間はキーワード追加のみに留め、
    // 成分抽出・A/B比較・URL改修・GA4自動フィードバック等の新機能開発はしない方針。
    "シニア犬 国産 無添加", "シニア猫 国産 無添加", "グレインフリー 国産 ドッグフード",
    "国産 無添加 犬 おやつ", "国産 無添加 猫 おやつ",
